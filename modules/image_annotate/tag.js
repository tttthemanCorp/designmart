/*
 * Update to Drupal 7
 * drupal.behaviors http://drupal.org/node/304258 (D6) and http://drupal.org/node/756722 
 *  and http://drupal.org/update/modules/6/7#drupal_behaviors 
 *
 * //@todo Percentage>pixels http://stackoverflow.com/questions/5230425/getting-percent-css-position-with-jquery
 * http://drupal.org/node/756722#comment-3652482 - "Drupal.settings.blah.setting" becomes "settings.blah.setting"
 * http://drupal.org/node/224333#local_settings_behaviors - Settings passed locally to JavaScript Behaviors
 *
 */
(function ($) { //In jQuery namespace

	Drupal.behaviors.imageAnnotate =  {
	  attach: function (context, settings) { //D7
		  for (var i=0; i<settings.imageAnnotate.length; i++) {
		    annotativeImage = new Drupal.annotativeImage(settings.imageAnnotate[i]);
		  }
	...
	Drupal.annotativeImage = function (image) {
	  ...
	  //this.canvas.height(this.image.height());  // returns 0 ?
	  //this.canvas.width(this.image.width());
	  this.canvas.height(image.imgheight);
	  this.canvas.width(image.imgwidth);
	  ...
	  this.canvas.children('.image-annotate-view, .image-annotate-edit').height(image.imgheight);
	  this.canvas.children('.image-annotate-view, .image-annotate-edit').width(image.imgwidth);
	 ...
	 * Add a note
	 */
	Drupal.annotativeImage.prototype.addNote = function(context, settings) {
	  if (this.mode == 'view') {
	    this.mode = 'edit';
	    var image = this;
	    ...
	    editable.note.load('/image-annotate/create/' + this.nid, {}, function() {
	      Drupal.behaviors.collapse.attach(editable.note);
	      var form = $('#image-annotate-edit-form form');
	      // remove these *EVIL* fixes
	      //form.attr('action', '/?q=comment/reply/'+ image.nid); /* Evil! */
	      ...
	      editable.note.append(cancel);
	      //$('#edit-actions #edit-submit').after(cancel);  - doesn't work
	 ...
	 */
	Drupal.imageAnnotation.prototype.edit = function(context, settings) {
	  if (this.image.mode == 'view') {
	    ...
	    // Load the form and set the draggable/resizable area
	    editable.note.load('/image-annotate/edit/' + note.aid, {}, function() {
	      Drupal.behaviors.collapse.attach(editable.note);
	      var form = $('#image-annotate-edit-form form');
	      // remove these *EVIL* fixes
	      //form.attr('action', '/?q=comment/'+ note.cid + '/edit'); /* Evil! */
	      //$('#image-annotate-edit-form input').attr('for', ''); /* Kicking babies evil! */
	      // Add the image note information to the form action on submission
	      form.submit(function() {
	        var areaFields = $('<input type="hidden" value="1" name="image-annotate"/>'+
